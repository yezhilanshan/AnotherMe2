...
export const ToolCallBlock = React.memo(function ToolCallBlock({ toolName, state, input, output, error }: ToolCallProps) {
  ...
  const stateConfig = {
    running: { icon: 'sync-outline' as const, color: colors.warning, label: '运行中' },
    completed: { icon: 'checkmark-circle-outline' as const, color: colors.success, label: '完成' },
    error: { icon: 'close-circle-outline' as const, color: colors.error, label: '错误' },
  };
    ...
    <View style={styles.container}>
      ...
      {expanded && (
        <View style={styles.content}>
          ...
          {error && (
            <View style={styles.section}>
              <Text style={[styles.sectionTitle, { color: colors.error }]}>错误</Text>
