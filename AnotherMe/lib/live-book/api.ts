import type { Book, BookDetail, BookProposal, Page, Spine, Block } from '@/lib/live-book/types';

// ── API URL helpers ──────────────────────────────────────────────────

const BASE = '/api/live-book';
...
export interface CreateBookPayload {
  ...
  language?: string;
  /** LLM provider config — sent as headers, not in body */
...
export const bookApi = {
  ...
  delete: (book_id: string) =>
    request<{ deleted: boolean; book_id: string }>(`/books/${encodeURIComponent(book_id)}`, {
      method: 'DELETE',
    }),
  ...
  create: (payload: CreateBookPayload) => {
    const { providerConfig, ...body } = payload;
    const extraHeaders: Record<string, string> = {};
    if (providerConfig?.apiKey) extraHeaders['X-LLM-Api-Key'] = providerConfig.apiKey;
    if (providerConfig?.model) extraHeaders['X-LLM-Model'] = providerConfig.model;
    if (providerConfig?.baseUrl) extraHeaders['X-LLM-Base-Url'] = providerConfig.baseUrl;
    return request<{ book: Book; proposal: BookProposal }>('/books', {
      method: 'POST',
  ...
  moveBlock: (book_id: string, page_id: string, block_id: string, new_position: number) =>
...
function getWebSocketUrl(): string {
  ...
  const envWsUrl = process.env.NEXT_PUBLIC_ANOTHERME2_GATEWAY_WS_URL?.trim();
  if (envWsUrl) return envWsUrl;
  const gatewayHost = process.env.NEXT_PUBLIC_ANOTHERME2_GATEWAY_HOST?.trim() || '127.0.0.1:8080';
  return `${protocol}//${gatewayHost}/live-book/ws`;
}
