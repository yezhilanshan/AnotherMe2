/**
 * Photo Manim Direct Capability Handler
 *
 * Simplified pipeline: upload image → call vision LLM → render Manim → return video.
 * Bypasses the complex AnotherMe2 multi-agent workflow.
 */

import type { CapabilityHandler, CapabilityRequest, CapabilityStageResult, CapabilityResult } from '../capability-runtime';

export interface PhotoManimDirectPayload {
  imageObjectKey: string;
  problemText?: string;
}

export interface PhotoManimDirectResult {
  success: boolean;
  jobId: string;
  status: string;
  step: string;
  progress: number;
  pollUrl: string;
  pollIntervalMs: number;
}

export const photoManimDirectHandler: CapabilityHandler<PhotoManimDirectPayload> = {
  capabilityId: 'photo_manim_direct',

  validatePayload(payload: unknown): PhotoManimDirectPayload {
    if (!payload || typeof payload !== 'object') {
      throw new Error('Invalid payload: expected object');
    }
    const p = payload as Record<string, unknown>;
    if (!p.imageObjectKey || typeof p.imageObjectKey !== 'string') {
      throw new Error('Invalid payload: imageObjectKey required');
    }
    return {
      imageObjectKey: p.imageObjectKey,
      problemText: typeof p.problemText === 'string' ? p.problemText : undefined,
    };
  },

  async *execute(request: CapabilityRequest<PhotoManimDirectPayload>): AsyncGenerator<CapabilityStageResult, CapabilityResult, unknown> {
    const startTime = Date.now();
    const {
      imageObjectKey,
      problemText,
    } = request.payload;

    // Stage: pre_process
    const preStart = Date.now();
    try {
      yield {
        stage: 'pre_process',
        success: true,
        output: { hasProblemText: !!problemText?.trim() },
        durationMs: Date.now() - preStart,
        completedAt: Date.now(),
      };
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error));
      yield {
        stage: 'pre_process',
        success: false,
        error: { code: 'PRE_PROCESS_FAILED', message: err.message },
        durationMs: Date.now() - preStart,
        completedAt: Date.now(),
      };
      throw err;
    }

    // Stage: agent_invoke (create the job via gateway)
    const invokeStart = Date.now();
    let jobResult: { job_id: string; status: string; step: string; progress: number };

    try {
      const { createAnotherMe2PhotoManimJob } = await import('@/lib/server/anotherme2-gateway');
      jobResult = await createAnotherMe2PhotoManimJob({
        imageObjectKey,
        ...(problemText ? { problemText } : {}),
      });
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error));
      yield {
        stage: 'agent_invoke',
        success: false,
        error: { code: 'AGENT_INVOKE_FAILED', message: err.message },
        durationMs: Date.now() - invokeStart,
        completedAt: Date.now(),
      };
      throw err;
    }

    yield {
      stage: 'agent_invoke',
      success: true,
      output: { jobId: jobResult.job_id, status: jobResult.status, step: jobResult.step },
      durationMs: Date.now() - invokeStart,
      completedAt: Date.now(),
    };

    // Stage: post_process
    const postStart = Date.now();
    yield {
      stage: 'post_process',
      success: true,
      output: { pipeline: 'photo_manim_direct' },
      durationMs: Date.now() - postStart,
      completedAt: Date.now(),
    };

    // Stage: persist
    const persistStart = Date.now();
    yield {
      stage: 'persist',
      success: true,
      output: { jobId: jobResult.job_id, wasAborted: false },
      durationMs: Date.now() - persistStart,
      completedAt: Date.now(),
    };

    // Stage: complete
    const completeStage: CapabilityStageResult = {
      stage: 'complete',
      success: true,
      output: {
        jobId: jobResult.job_id,
        status: jobResult.status,
        step: jobResult.step,
        progress: jobResult.progress,
      },
      durationMs: Date.now() - startTime,
      completedAt: Date.now(),
    };
    yield completeStage;

    return {
      success: true,
      output: {
        jobId: jobResult.job_id,
        status: jobResult.status,
        step: jobResult.step,
        progress: jobResult.progress,
      },
      stages: [],
      traceEvents: [],
      totalDurationMs: Date.now() - startTime,
    };
  },
};
