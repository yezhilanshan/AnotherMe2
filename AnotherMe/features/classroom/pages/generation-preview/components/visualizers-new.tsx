...
function PdfScanVisualizer() {
  return (
    <div className="size-32 relative flex items-center justify-center">
      {/* Background glow */}
      <motion.div
        className="absolute inset-2 bg-sky-500/5 rounded-2xl blur-lg"
      ...
      <div className="w-20 h-28 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 shadow-xl relative overflow-hidden">
        ...
        {/* Scanning laser - vertical sweep (different from OpenMAIC's horizontal) */}
        <motion.div
          className="absolute inset-x-0 h-[2px] bg-gradient-to-r from-transparent via-sky-400 to-transparent shadow-[0_0_12px_rgba(56,189,248,0.6)]"
          animate={{ top: ['5%', '90%', '5%'] }}
      ...
      >
        <ScanLine className="size-6 text-sky-500/70" />
      </motion.div>
    </div>
  );
}

// Web Search: Search results with animated highlights
function WebSearchVisualizer({ sources }: { sources: Array<{ title: string; url: string }> }) {
  ...
  return (
    <div className="size-56 relative flex items-center justify-center">
      {/* Background glow */}
      <motion.div
        className="absolute inset-0 blur-3xl rounded-full bg-cyan-500/8"
      ...
      <div className="w-44 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden relative">
        ...
        <div className="p-2 space-y-0.5 relative">
          ...
          {sources.length > 0 && (
            <motion.div
              className="absolute left-2 right-2 rounded-lg bg-cyan-500/[0.06] dark:bg-cyan-400/[0.08]"
          ...
          {sources.length === 0
            ? skeletonResults.map((item, i) => (
                ...
                >
                  <div
                    className="h-1.5 bg-cyan-200/40 dark:bg-cyan-800/30 rounded"
            ...
            : sources.slice(0, 4).map((source, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-[6px] text-cyan-500/50 truncate leading-tight">
      ...
      {sources.length > 0 && (
        <motion.div
          ...
          className="absolute -top-2 -right-2 h-6 px-2 rounded-full bg-cyan-500 text-white text-[10px] font-bold flex items-center justify-center shadow-lg shadow-cyan-500/25 z-20 gap-0.5"
        >
...
// Outline: Document with streaming text lines
  ...
  return (
    <div className="w-40 h-52 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-2xl p-4 overflow-hidden relative rotate-[2deg] hover:rotate-0 transition-transform duration-500">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <div className="w-1/3 h-2 bg-slate-100 dark:bg-slate-700 rounded" />
      </div>
          ...
          allLines.map((line, i) => (
            <motion.div
              ...
              className={cn(
                'truncate',
                !line.startsWith('   ')
                  ? 'text-violet-600 dark:text-violet-400 font-semibold text-[9px]'
                  : 'pl-1 opacity-80',
              )}
            >
              {line}
      ...
      <motion.div
        className="absolute bottom-3 right-3 size-2 bg-violet-500 rounded-full"
...
// Agent Generation: Floating agent cards
    ...
    <div className="w-60 h-40 mx-auto flex items-center justify-center">
      <div className="flex gap-3">
        {[0, 1, 2].map((i) => (
          <motion.div
            key={i}
            className="w-14 h-20 rounded-lg bg-gradient-to-br from-fuchsia-400 to-violet-500 dark:from-fuchsia-600 dark:to-violet-700 shadow-lg"
            animate={{ y: [0, -10, 0], rotateZ: [0, -3, 3, 0] }}
            transition={{
              duration: 1.8,
              repeat: Infinity,
              delay: i * 0.4,
...
// Content: Cycles through content types
    ...
    <div className="size-56 relative flex items-center justify-center perspective-[800px]">
      ...
      {[0, 1].map((i) => (
        <motion.div
          ...
          animate={{ rotate: 360 }}
          transition={{ duration: 40 + i * 15, ease: 'linear', repeat: Infinity, delay: i * -5 }}
        />
      ))}
              ...
              <div className="flex flex-col h-full justify-center space-y-2 pt-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[0, 1, 2, 3].map((i) => (
                    ...
                    >
                      <div
                        className={cn('size-1.5 rounded-full mr-2', i === 1 ? 'bg-white' : 'bg-slate-300')}
                      />
                      <div
                        className={cn(
            ...
            {index === 3 && (
              <div className="flex flex-col h-full relative pt-1">
                ...
                <div className="flex-1 flex gap-2 relative">
                  ...
                  >
                    {[1, 2, 3].map((i) => (
                      <div key={i} className="h-1 w-full bg-slate-200 dark:bg-slate-600 rounded-full" />
                    ))}
                  </motion.div>
...
// Actions: Timeline of actions being orchestrated
function ActionsVisualizer() {
  const [activeIdx, setActiveIdx] = useState(0);

  const actionItems = [
    { icon: MessageSquare, label: 'Speech', color: 'text-rose-500' },
    { icon: Focus, label: 'Spotlight', color: 'text-amber-500' },
    { icon: MessageSquare, label: 'Speech', color: 'text-rose-500' },
    { icon: Play, label: 'Interact', color: 'text-teal-500' },
    { icon: MessageSquare, label: 'Speech', color: 'text-rose-500' },
  ];
  ...
  return (
    <div className="size-56 relative flex items-center justify-center">
      {/* Background pulse */}
      <motion.div
        className="absolute inset-0 blur-3xl rounded-full bg-rose-500/8"
        animate={{ scale: [1, 1.15, 1], opacity: [0.3, 0.5, 0.3] }}
        transition={{ duration: 3.5, repeat: Infinity }}
      />
      ...
      <div className="w-44 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden relative">
        {/* Header */}
        <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-700 flex items-center gap-2">
          <Clapperboard className="size-3 text-rose-500" />
          <motion.div
            initial={{ width: 0 }}
            animate={{ width: '50%' }}
            transition={{ delay: 0.2 }}
            className="h-1.5 bg-rose-500/20 rounded-full"
          />
        </div>
        ...
        <div className="p-2 space-y-1.5 relative">
          <motion.div
            className="absolute left-2 right-2 rounded-lg bg-rose-500/[0.06] dark:bg-rose-400/[0.08]"
            style={{ height: ROW_H - 6 }}
