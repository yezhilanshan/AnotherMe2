...
import { retryMediaTask, confirmSofteningAndRetry, rejectSoftening } from '@/lib/media/media-orchestrator';
import { RotateCcw, Film, ShieldAlert, VideoOff, Check, X } from 'lucide-react';
...
export function BaseVideoElement({ elementInfo }: BaseVideoElementProps) {
  ...
  const showNeedsConfirmation = isPlaceholder && task?.status === 'needs_confirmation';
    ...
    >
      ...
      >
        {showDisabled ? (
          ...
          <div className="w-full h-full bg-amber-50 dark:bg-amber-900/20 flex flex-col items-center justify-center gap-2 rounded p-2">
            ...
            <p className="text-[9px] text-amber-700 dark:text-amber-300 text-center leading-tight max-w-full line-clamp-2">
              {t('settings.mediaSoftenPromptHint')}
            </p>
            <div className="flex items-center gap-1.5">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  confirmSofteningAndRetry(elementInfo.src);
                }}
                onPointerDown={(e) => e.stopPropagation()}
                className="flex items-center gap-0.5 px-2 py-0.5 text-[10px] font-medium text-green-700 dark:text-green-300 bg-green-100 dark:bg-green-900/40 rounded hover:bg-green-200 dark:hover:bg-green-900/60 transition-colors"
              >
                <Check className="w-3 h-3" />
                {t('settings.mediaConfirmSoften')}
              </button>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  rejectSoftening(elementInfo.src);
                }}
                onPointerDown={(e) => e.stopPropagation()}
                className="flex items-center gap-0.5 px-2 py-0.5 text-[10px] font-medium text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800/40 rounded hover:bg-gray-200 dark:hover:bg-gray-700/60 transition-colors"
              >
                <X className="w-3 h-3" />
                {t('settings.mediaCancel')}
              </button>
            </div>
          </div>
        ) : showError ? (
