...
export default function CalendarPage() {
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-1 bg-card border border-border rounded-md p-1 shadow-sm">
            ...
            <button
              onClick={jumpToToday}
              className="px-3 py-1 text-xs font-medium text-foreground hover:bg-muted rounded transition-colors"
            >
              今天
            </button>
  ...
  const renderDays = () => {
    ...
    for (let i = 0; i < 7; i += 1) {
      days.push(
        <div
          key={i}
          className="text-xs font-bold text-muted-foreground tracking-wider text-center py-3 border-b border-border"
        >
    ...
    return (
      <div className="grid grid-cols-7 bg-muted rounded-t-xl border border-border border-b-0">
        {days}
      </div>
    );
  };
  ...
  const renderCells = () => {
            ...
            <div className="space-y-1.5">
              {dayTasks.map((task) => (
                <a
                  ...
                  href={
                    task.classroomId
                      ? `/preview-class?classroomId=${encodeURIComponent(task.classroomId)}`
                      : '#'
                  }
                  className={cn(
    ...
    return (
      <div className="border-l border-t border-border rounded-b-xl overflow-hidden">{rows}</div>
    );
  };
