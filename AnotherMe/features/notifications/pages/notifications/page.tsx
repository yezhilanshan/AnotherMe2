    ...
    async function loadData() {
      try {
        const [classResult, healthResult] = await Promise.allSettled([
          fetch('/api/classroom?limit=20', { method: 'GET', cache: 'no-store' }).then(
            async (resp) => {
        ...
        <div className="divide-y divide-gray-100">
          {notifications.map((notification) => {
            ...
            return (
              ...
              >
                <div
                  className={`h-12 w-12 rounded-full flex items-center justify-center shrink-0 ${bg}`}
                >
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between mb-1">
                    <h3
                      className={`text-sm font-bold text-foreground ${!notification.read ? 'flex items-center gap-2' : ''}`}
                    >
                      {notification.title}
                      {!notification.read ? (
