...
import { getCoWriterDocument, updateCoWriterDocument } from '@/lib/co-writer-api';
import { notifyCoWriterChanged } from '@/lib/co-writer-events';
import { CO_WRITER_SAMPLE_TEMPLATE } from './sampleTemplate';

// i18n no-op — all strings are already Chinese
const t = (s: string, _vars?: Record<string, string | number>) => s;

// Stub: notebook save is not yet available in AnotherMe
...
function SaveToNotebookModal(_props: {
  isOpen: boolean;
...
// Stub: knowledge base listing not yet available
async function listKnowledgeBases(): Promise<Array<{ name: string; is_default?: boolean }>> {
  return [];
}
...
type ToolName = 'brainstorm' | 'rag' | 'web_search' | 'code_execution' | 'reason' | 'paper_search';
...
const SPLIT_RATIO_KEY = 'anotherme.co_writer.split_ratio';
const SYNC_SCROLL_KEY = 'anotherme.co_writer.sync_scroll';
const LOCAL_DRAFT_PREFIX = 'anotherme.co_writer.draft.';
...
export default function CoWriterPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const defaultKb =
          list.find((k: KnowledgeBase) => k.is_default)?.name || list[0]?.name || '';
  ...
  const measureSelectionAnchor = useCallback((textarea: HTMLTextAreaElement, index: number) => {
  ...
  }, [hideSelectionPopover, markdown, measureSelectionAnchor, selectionPopoverPinned]);
  ...
  const handleOpenSaveToNotebook = useCallback(() => {
    if (!markdown.trim()) {
      setError('保存到笔记本前请先添加内容。');
      return;
    }
    const fallbackTitle = '未命名协作写作文档';
  ...
  const applyReactSelectionEdit = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/co-writer/edit_react/stream', {
        method: 'POST',
  ...
  const applyEdit = async () => {
    ...
    setStatus('');
    try {
      const response = await fetch('/api/co-writer/edit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          text: markdown,
  ...
  const applyAutoMark = async () => {
    ...
    setStatus('');
    try {
      const response = await fetch('/api/co-writer/automark', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  }, [buildJointMarkers, releaseScrollSyncSource, syncScrollEnabled, updateSelectionPopover]);
  ...
  if (docNotFound) {
    return (
      <div className="flex h-full min-h-full flex-col items-center justify-center gap-4 bg-[var(--background)] p-10 text-center">
        <p className="text-lg font-medium text-[var(--foreground)]">{'文档未找到'}</p>
    ...
    <div className="flex h-full min-h-full flex-col overflow-hidden bg-[var(--background)]">
      ...
      <header className="flex shrink-0 items-center justify-between border-b border-[var(--border)] px-4 py-1.5">
        <div className="flex min-w-0 items-center gap-3 text-sm text-[var(--muted-foreground)]">
          ...
          <span className="hidden text-xs sm:inline">
            {wordCount} {'词'} &middot; {charCount} {'字符'}
          </span>
          {isSavingDoc ? (
        ...
        <div className="flex items-center gap-2">
          <ToolbarIconBtn title={'清空'} onClick={clearDocument}>
            <Eraser size={17} strokeWidth={1.7} />
          </ToolbarIconBtn>
          <ToolbarIconBtn title={'导出 Markdown'} onClick={handleDownload}>
            <Download size={17} strokeWidth={1.7} />
          </ToolbarIconBtn>
          <ToolbarIconBtn title={'保存到笔记本'} onClick={handleOpenSaveToNotebook}>
            <NotebookPen size={17} strokeWidth={1.7} />
          </ToolbarIconBtn>
          <ToolbarIconBtn title={'加载示例模板'} onClick={loadExampleTemplate}>
      ...
      <div className="flex shrink-0 items-center gap-0.5 overflow-x-auto border-b border-[var(--border)] px-3 py-1">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1.5 pl-3 text-[10px] text-[var(--muted-foreground)]">
          <button
            ...
            title={syncScrollEnabled ? '滚动同步已开启，点击关闭' : '滚动同步已关闭，点击开启'}
          ...
          <span className="rounded bg-[var(--muted)] px-1.5 py-0.5">Mermaid</span>
        </div>
      </div>
      ...
      >
        ...
        {showEditor && (
          ...
          >
            <div className="flex shrink-0 items-center justify-between border-b border-[var(--border)] px-3 py-1">
              <span className="text-xs font-medium text-[var(--muted-foreground)]">{'编辑器'}</span>
              <button
                title={'折叠编辑器'}
        ...
        {showPreview && (
          ...
          >
            <div className="flex shrink-0 items-center justify-between border-b border-[var(--border)] px-3 py-1">
              <span className="text-xs font-medium text-[var(--muted-foreground)]">{'预览'}</span>
              <button
                title={'折叠预览'}
          ...
          {(isEditing || selectionTrace) && (
            <div className="mt-2 rounded-xl border border-[var(--border)]/70 bg-[var(--muted)]/18">
              ...
              >
                ...
                />
                <span className="font-medium text-[var(--foreground)]">{'追踪'}</span>
                ...
                >
                  {selectionTrace?.thinking ? (
                    <div className="space-y-1.5">
                      ...
                      <MarkdownRenderer content={selectionTrace.thinking} variant="trace" />
                    </div>
                  ) : isEditing ? (
                    <div className="opacity-70">{'思考中…'}</div>
                  ) : null}
                        ...
                        {selectionTrace.toolTraces.map((trace, index) => (
                          <div key={`${trace.name}-${index}`} className="space-y-1">
                            <div>
                              ...
                              <span className="text-[var(--foreground)]">{trace.name}</span>
                            </div>
                  ...
                  !selectionTrace.response ? (
                    <div className="mt-2 opacity-70">{'运行工具并准备最终编辑…'}</div>
                  ) : null}
                </div>
              )}
            </div>
        ...
        >
          <div className="w-full max-w-md rounded-xl border border-[var(--border)] bg-[var(--card)] shadow-xl">
            <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3">
              <h2 className="text-sm font-semibold text-[var(--foreground)]">{'全文 AI 编辑'}</h2>
              <button
      ...
      <SaveToNotebookModal
        isOpen={notebookSavePayload !== null}
