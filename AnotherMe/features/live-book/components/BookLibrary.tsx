...
import type { Book, BookStatus } from "@/lib/live-book/types";
...
function relativeTime(seconds: number): string {
  ...
  const timestampSeconds = seconds > 10_000_000_000 ? seconds / 1000 : seconds;
  const diff = Math.max(0, Date.now() / 1000 - timestampSeconds);
                    ...
                    <button
                      ...
                      className={`absolute right-2 top-2 rounded-md p-1.5 transition-colors ${
                        isPendingDelete
                          ...
                          : "bg-white/60 text-[var(--muted-foreground)] opacity-100 backdrop-blur-sm hover:bg-rose-500/10 hover:text-rose-600 sm:opacity-0 sm:group-hover:opacity-100 dark:bg-black/30 dark:hover:text-rose-400"
                      }`}
                    >
                      <Trash2 size={13} />
                    </button>
