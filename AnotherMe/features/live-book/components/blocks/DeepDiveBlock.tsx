...
import { ChevronRight, ExternalLink, Sparkles, Loader2 } from "lucide-react";
  ...
  return (
    <div className="rounded-2xl border border-[var(--primary)]/30 bg-gradient-to-br from-[var(--primary)]/5 to-transparent p-4 shadow-sm">
      ...
      {linkedPageId && onNavigateToPage && (
        <button
          onClick={() => onNavigateToPage(linkedPageId)}
          className="mt-3 flex min-h-[44px] w-full items-center justify-center gap-2 rounded-lg border border-[var(--primary)]/30 bg-[var(--primary)]/5 px-4 py-2.5 text-xs font-medium text-[var(--primary)] active:bg-[var(--primary)]/10"
        >
          <ExternalLink className="h-4 w-4" />
          View linked sub-page
        </button>
      )}
    </div>
  );
