...
import { LiveBookSourcePicker, type LiveBookSourceInput } from '@/features/live-book/pages/live-book/live-book-source-picker';
import { getLiveBookHeaders, initialForm, parseApi } from '@/features/live-book/pages/live-book/api';
import {
  BLOCK_TYPE_ICON,
  CHANGEABLE_BLOCK_TYPES,
  INSERTABLE_BLOCK_TYPES,
  blockTypeAccent,
  blockTypeColor,
  blockTypeLabel,
  extractBlockBridgeText,
  extractBlockSourceLabels,
  manualBlockDefaults,
  pageStatusBadge,
  renderBlockPayload,
  statusColor,
  statusText,
} from '@/features/live-book/pages/live-book/block-rendering';
import type {
  BlockType,
  FormState,
  Insights,
  ...
  LiveBookSummary,
  ViewMode,
} from '@/features/live-book/pages/live-book/types';
...
export default function LiveBookPage() {
  ...
  const handleCompilePage = useCallback(
    async (pageId: string) => {
      if (!detail) return;
      setCompiling(true);
      try {
    ...
    [detail, loadHealth, loadInsights],
  );
  ...
  }, [detail, selectedPage, compiling, handleCompilePage]);
  ...
  return (
    <div className="flex h-[calc(var(--app-dvh)-4rem)] -m-4 bg-[#F3F2EE] max-md:h-mobile-app md:-m-8">
      {renderSidebar()}
