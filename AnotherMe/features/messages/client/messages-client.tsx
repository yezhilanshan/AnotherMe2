...
import {
  ...
  mapBackendMessage,
  parseMemberIds,
} from '@/features/messages/pages/messages/utils';
...
export default function MessagesPage() {
  ...
  const filteredContacts = useMemo(() => {
    ...
    return contacts.filter(
      (c) => c.name.toLowerCase().includes(query) || (c.type === 'group' && '群聊'.includes(query)),
    );
  ...
  const ensureAiSession = async (conversationId: string, conversationName?: string) => {
    ...
    setAiSessionByConversation((prev) => ({
      ...prev,
      [conversationId]: createdPayload.session!.session_id,
    }));
          ...
          {filteredContacts.length === 0 ? (
            ...
            <div className="space-y-1">
              {filteredContacts.map((contact) => {
                ...
                const isCreator = contact.creator_id === currentUserId;
                const avatarColor = getAvatarColor(contact.conversation_id);
              ...
              <div className="flex items-center gap-3">
                ...
                <div
                  className={cn(
                    ...
                    getAvatarColor(selectedContact.conversation_id),
                  )}
                >
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden min-h-0 p-4 space-y-4">
              {activeMessages.length === 0 ? (
                ...
                activeMessages.map((msg, index) => {
                  const showAvatar = index === 0 || activeMessages[index - 1].role !== msg.role;
