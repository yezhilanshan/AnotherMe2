...
class PhotoManimDirectInput(BaseModel):
    """Input for the photo-manim-direct pipeline."""
...
def validate_job_payload(job_type: JobType, payload: Dict[str, Any]) -> Dict[str, Any]:
    model_map = {
        ...
        JobType.PHOTO_MANIM_DIRECT: PhotoManimDirectInput,
    ...
    if job_type == JobType.STUDY_PACKAGE_GENERATE:
        ...
        if (
            source["type"] == "photo"
            and not (source.get("image_object_key") or "").strip()
        ):
            raise ValueError(
                "source.image_object_key is required when source.type=photo"
            )
        if not outputs.get("course") and not outputs.get("problem_video"):
            raise ValueError(
                "outputs.course and outputs.problem_video cannot both be false"
            )
