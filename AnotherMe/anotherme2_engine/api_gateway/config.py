...
class Settings:
    ...
    queue_backend: Literal["auto", "redis", "polling"] = os.getenv(
        "GATEWAY_QUEUE_BACKEND", "auto"
    ...
    running_job_stale_seconds: int = int(
        os.getenv("GATEWAY_RUNNING_JOB_STALE_SECONDS", "1800")
    )
    running_job_recover_batch: int = int(
        os.getenv("GATEWAY_RUNNING_JOB_RECOVER_BATCH", "8")
    )
    running_job_result_reconcile_batch: int = int(
        os.getenv("GATEWAY_RUNNING_JOB_RESULT_RECONCILE_BATCH", "12")
    )
    missing_input_cleanup_batch: int = int(
        os.getenv("GATEWAY_MISSING_INPUT_CLEANUP_BATCH", "12")
    )
    purge_prestart_jobs_on_startup: bool = _bool_env(
        "GATEWAY_PURGE_PRESTART_JOBS_ON_STARTUP", False
    )
    purge_prestart_jobs_batch: int = int(
        os.getenv("GATEWAY_PURGE_PRESTART_JOBS_BATCH", "5000")
    )
    ...
    anotherme_model: str = os.getenv("ANOTHERME_MODEL", "")
    anotherme_api_key: str = os.getenv("ANOTHERME_API_KEY", "")
    anotherme_api_base_url: str = os.getenv("ANOTHERME_API_BASE_URL", "")
    ...
    object_storage_bucket: str = os.getenv(
        "OBJECT_STORAGE_BUCKET", "anotherme2-artifacts"
    )
    ...
    local_storage_root: str = _runtime_path_env(
        "LOCAL_STORAGE_ROOT", "./gateway_data/objects"
    )

    worker_temp_root: str = _runtime_path_env(
        "GATEWAY_WORKER_TEMP_ROOT", "./gateway_data/tmp"
    )
    worker_output_root: str = _runtime_path_env(
        "GATEWAY_WORKER_OUTPUT_ROOT", "./gateway_data/runs"
    )
    ...
    def __post_init__(self) -> None:
        for field_name in (
            ...
            object.__setattr__(
                self, field_name, _resolve_runtime_path(getattr(self, field_name))
            )
