...
class ProgressTracker:
    ...
    def _notify(self, progress: dict):
        ...
        from tutor_engine.runtime.mode import is_server

        if is_server():
            try:
                from tutor_engine.api.utils.progress_broadcaster import ProgressBroadcaster
    ...
    def update(
        ...
        try:
            ...
            fallback_logger = logging.getLogger("tutor_engine.ProgressTracker")
            prefix = f"[{self.task_id}]" if self.task_id else ""
        ...
        if self.task_id:
            try:
                from tutor_engine.api.utils.task_log_stream import get_task_stream_manager
