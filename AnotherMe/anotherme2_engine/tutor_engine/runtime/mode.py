...
Controls whether Tutor Engine is running as a CLI application or an API server.
...
def _resolve_mode() -> RunMode:
    raw = os.environ.get("TUTOR_ENGINE_MODE", "").strip().lower()
...
def set_mode(mode: RunMode) -> None:
    ...
    global _current_mode
    _current_mode = mode
    os.environ["TUTOR_ENGINE_MODE"] = mode.value
