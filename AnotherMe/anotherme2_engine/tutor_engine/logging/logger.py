...
from tutor_engine.config.constants import PROJECT_ROOT
...
class Logger:
    """
    Unified logger for Tutor Engine.
    ...
    def __init__(
        ...
        self.logger = logging.getLogger(f"tutor_engine.{name}")
        ...
        if file_output:
            ...
            log_file = log_dir_path / f"tutor_engine_{timestamp}.log"
...
def get_logger(
    ...
    if log_dir is None or effective_level is None:
        try:
            from tutor_engine.services.config import get_path_from_config, load_config_with_main
