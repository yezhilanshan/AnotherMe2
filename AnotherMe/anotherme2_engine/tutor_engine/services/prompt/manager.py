...
from tutor_engine.services.config import PROJECT_ROOT, parse_language
...
class PromptManager:
    ...
    # Modules that are not under tutor_engine/agents/ directory
    # Map module_name → on-disk path component under tutor_engine/
    ...
    def _candidate_prompt_dirs(self, module_name: str) -> list[Path]:
        ...
        if module_name in self.NON_AGENT_MODULES:
            ...
            current_dir = PROJECT_ROOT / "tutor_engine" / module_name / "prompts"
            return [legacy_dir, current_dir]

        legacy_dir = PROJECT_ROOT / "src" / "agents" / module_name / "prompts"
        current_dir = PROJECT_ROOT / "tutor_engine" / "agents" / module_name / "prompts"
        return [legacy_dir, current_dir]
