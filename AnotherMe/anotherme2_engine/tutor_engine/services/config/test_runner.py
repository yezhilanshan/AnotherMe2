...
from tutor_engine.services.llm.client import reset_llm_client
...
class ConfigTestRunner:
    ...
    async def _test_llm(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from tutor_engine.services.llm import clear_llm_config_cache, get_token_limit_kwargs
        from tutor_engine.services.llm import complete as llm_complete
        from tutor_engine.services.llm.config import LLMConfig
    ...
    async def _test_embedding(
        ...
        from tutor_engine.services.embedding.client import EmbeddingClient
        from tutor_engine.services.embedding.config import EmbeddingConfig
        ...
        vectors = await client.embed(["Tutor Engine embedding smoke test"])
    ...
    def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from tutor_engine.services.search import web_search
        ...
        run.emit("info", "Running search query: Tutor Engine configuration health check")
        result = web_search("Tutor Engine configuration health check", provider=provider)
        run.emit(
            "response",
