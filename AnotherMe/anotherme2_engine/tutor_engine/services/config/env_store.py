...
def _resolve_env_path() -> Path:
    """Pick the best available .env file: api_gateway/.env > project .env."""
    if GATEWAY_ENV_PATH.exists():
        return GATEWAY_ENV_PATH
    return ENV_PATH


class EnvStore:
    """Canonical `.env` reader/writer for local Tutor Engine configuration."""

    def __init__(self, path: Path | None = None):
        self.path = path or _resolve_env_path()
