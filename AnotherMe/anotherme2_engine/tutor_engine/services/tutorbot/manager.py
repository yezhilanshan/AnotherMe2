...
Each TutorBot instance runs as a set of asyncio tasks within the Tutor Engine
...
class TutorBotManager:
    ...
    def _shared_memory_dir(self) -> Path:
        """Public memory shared by Tutor Engine and all bots."""
    ...
    async def start_bot(self, bot_id: str, config: BotConfig | None = None) -> TutorBotInstance:
        ...
        from tutor_engine.tutorbot.agent.loop import AgentLoop
        from tutor_engine.tutorbot.bus.queue import MessageBus
        from tutor_engine.tutorbot.config.schema import ExecToolConfig
        from tutor_engine.tutorbot.providers.tutor_provider import create_tutor_provider
        from tutor_engine.tutorbot.session.manager import SessionManager

        provider = create_tutor_provider()
        bus = MessageBus()
        ...
        # -- Heartbeat --------------------------------------------------------
        from tutor_engine.tutorbot.heartbeat import HeartbeatService
    ...
    async def _outbound_router(self, bot_id: str, bus: Any, instance: TutorBotInstance) -> None:
        ...
        """
        try:
            from tutor_engine.events.event_bus import Event, EventType, get_event_bus
            from tutor_engine.tutorbot.bus.events import OutboundMessage as _OMsg
    ...
    def _build_channel_manager(
        ...
        *,
        ...
        if not config.channels:
            return None
        from tutor_engine.tutorbot.channels.manager import ChannelManager
        from tutor_engine.tutorbot.config.schema import ChannelsConfig
    ...
    async def send_message(
        ...
        if instance.channel_manager and response:
            from tutor_engine.tutorbot.bus.events import OutboundMessage
