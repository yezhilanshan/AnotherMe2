"""
Shared prompt loader for non-BaseAgent call sites in ``tutor_engine/book``.

All LLM-facing prompts inside this module live as YAML under
``tutor_engine/book/prompts/{en,zh}/<name>.yaml`` and are loaded through the
unified :class:`~tutor_engine.services.prompt.PromptManager`. This helper is the
...
def load_book_prompts(name: str, language: str) -> dict[str, Any]:
    ...
    Args:
        name: File stem under ``tutor_engine/book/prompts/{lang}/``
    ...
    if not prompts:
        raise RuntimeError(
            ...
            f"Expected tutor_engine/book/prompts/{{en,zh}}/{name}.yaml."
        )
    return prompts
