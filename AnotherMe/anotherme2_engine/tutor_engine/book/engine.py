...
from tutor_engine.logging import get_logger
...
def _timeout_from_env(name: str, default: float) -> float:
    ...
    try:
        ...
        logger.warning(f"Invalid {name}={raw!r}; using {default}s")
        return default


async def _await_with_optional_timeout(awaitable, timeout_s: float):
    if timeout_s <= 0:
...
class BookEngine:
    ...
    async def confirm_proposal(
        ...
        *,
        ...
        async with bstream.stage(STAGE_SPINE):
            print("[DEBUG] engine.confirm_proposal: stage SPINE entered", flush=True)
            ...
            try:
                async with bstream.stage(STAGE_EXPLORATION):
                    ...
                    exploration = await _await_with_optional_timeout(
                        explorer.explore(
                            book_id=book.id,
                            proposal=proposal,
                            inputs=inputs,
                        ),
                        _timeout_from_env("BOOK_ENGINE_EXPLORATION_TIMEOUT_S", 30.0),
                    )
                ...
                logger.warning(f"SourceExplorer timed out for {book.id}")
                exploration = None
                await bstream.progress(
                    "Source exploration timed out — falling back to proposal-only spine.",
                    stage=STAGE_EXPLORATION,
                )
            ...
            async def _on_round(label: str, payload: dict[str, Any]) -> None:
                stage = (
                    STAGE_CRITIQUE if label.startswith("critique") else STAGE_SYNTHESIS
                )
            ...
            async with bstream.stage(STAGE_SYNTHESIS):
                ...
                try:
                    spine = await _await_with_optional_timeout(
                        ...
                        _timeout_from_env("BOOK_ENGINE_SPINE_TIMEOUT_S", 60.0),
                    )
                except asyncio.TimeoutError:
                    logger.warning(f"SpineSynthesizer timed out for {book.id}")
                    await bstream.progress(
                        "Spine synthesis timed out — using an editable draft outline.",
                        stage=STAGE_SYNTHESIS,
                    )
                    spine = self._fallback_spine(book_id=book.id, proposal=proposal)
                except Exception as exc:
                    logger.warning(
                        f"SpineSynthesizer failed for {book.id}: {exc}",
                        exc_info=True,
                    )
                    await bstream.progress(
                        "Spine synthesis failed — using an editable draft outline.",
                        stage=STAGE_SYNTHESIS,
                    )
                    spine = self._fallback_spine(book_id=book.id, proposal=proposal)
    ...
    def _fallback_spine(self, *, book_id: str, proposal: BookProposal) -> Spine:
        """Build a deterministic editable outline when LLM spine synthesis stalls."""
        raw_count = proposal.estimated_chapters or 0
        chapter_count = min(8, max(3, raw_count if raw_count > 0 else 5))
        zh = proposal.title and any("\u4e00" <= ch <= "\u9fff" for ch in proposal.title)
        title = (proposal.title or ("活书" if zh else "Live Book")).strip()
        description = (proposal.description or proposal.scope or title).strip()

        if zh:
            templates = [
                ("基础概览", ContentType.THEORY, "建立整体框架，明确学习范围。"),
                ("核心概念", ContentType.CONCEPT, "拆解关键概念及其关系。"),
                ("关键方法", ContentType.DERIVATION, "理解主要方法、推导或机制。"),
                ("应用练习", ContentType.PRACTICE, "通过例题和任务完成迁移应用。"),
                ("总结拓展", ContentType.THEORY, "回顾重点，并连接后续拓展方向。"),
            ]
            objective_prefix = "理解"
        else:
            templates = [
                ("Foundations", ContentType.THEORY, "Build the overall frame and scope."),
                ("Core concepts", ContentType.CONCEPT, "Break down the key ideas and relationships."),
                ("Key methods", ContentType.DERIVATION, "Understand the main methods or mechanisms."),
                ("Practice and applications", ContentType.PRACTICE, "Apply the ideas through examples and tasks."),
                ("Review and extensions", ContentType.THEORY, "Review the essentials and connect next steps."),
            ]
            objective_prefix = "Understand"

        chapters: list[Chapter] = []
        for idx in range(chapter_count):
            label, content_type, summary = templates[idx] if idx < len(templates) else (
                (f"专题 {idx + 1}" if zh else f"Topic {idx + 1}"),
                ContentType.THEORY,
                description,
            )
            chapter_title = f"{title}：{label}" if zh else f"{title}: {label}"
            chapters.append(
                Chapter(
                    title=chapter_title,
                    learning_objectives=[
                        f"{objective_prefix} {label}",
                        description[:160],
                    ],
        ...
        edges = [
            ConceptEdge(
                ...
                dst=nodes[idx + 1].id,
                relation="depends_on",
                rationale="fallback sequential outline",
            )
        ...
        return Spine(
            ...
            exploration_summary=(
                "LLM spine synthesis timed out; generated an editable fallback outline."
            ),
        )
    ...
    async def compile_page(
        ...
        *,
        ...
        if book is None or spine is None or page is None:
            raise ValueError(
                f"Cannot compile page – missing book/spine/page ({book_id}/{page_id})"
            )
        ...
        *,
        ...
        if not is_correct:
            ...
            if (
                page
                and page.chapter_id
                and page.chapter_id not in progress.weak_chapters
            ):
